...
def main() -> None:
    ...
    print(
        f"{'Date':<12} {'Symbol':<8} {'Side':<6} {'Qty':>6} {'Price':>10} {'PnL':>12}"
    )
    print("-" * 60)
    for t in sorted(trades, key=lambda x: x.timestamp):
        ...
        print(
            f"{date_str:<12} {t.symbol:<8} {side:<6} {abs(t.quantity):>6} {t.price:>10.2f} {pnl:>+12.2f}"
        )
    ...
    for t in losing[:15]:
        ...
        pnl = getattr(t, "realized_pnl", 0) or 0
        print(
            f"  {date_str} {t.symbol:<8} qty={t.quantity:>6} price={t.price:>10.2f} PnL={pnl:>+12.2f}"
        )
