...
# ---------------------------------------------------------------------------
# Integration: CPCV with real FeaturePipeline + XGBoost
# ---------------------------------------------------------------------------


class TestCPCVIntegration:
    def test_end_to_end_with_xgboost(self) -> None:
        """Full CPCV pipeline: features -> XGBoost -> CPCV evaluation."""
        ...
        assert cpcv_result.n_paths == 5
        assert len(cpcv_result.scores) == 5
        assert cpcv_result.mean_score > 0
        assert cpcv_result.std_score >= 0
        assert cpcv_result.predictions is not None
        assert len(cpcv_result.predictions) > 0

        # All scores should be valid (between 0 and 1 for accuracy)
        for score in cpcv_result.scores:
